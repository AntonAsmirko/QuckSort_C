...
struct my_simple_vector *read_data(char *file)
{
    ...
    while (1)
    {
        ...
        if (read_int(fp, &((*person).phone_number)))
        {
...
void quick_sort(struct my_simple_vector *vector, int l, int h, int (*comparator)(struct data *first, struct data *second),
                int depth)
{
    if (depth > 3500)
    {
...
void iterative_quick_sort(struct my_simple_vector *vector, int l, int h,
                          int (*comparator)(struct data *first, struct data *second))
{
    int stack[h - l + 1];
    int top = -1;
